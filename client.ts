...
export class LaftelClient {
    ...
    async login(email: string, password: string): Promise<boolean> {
        const res = await this._request<Raw.AuthResponse>("/authentications/v3/email/", {
            method: "POST",
    ...
    async getEpisodes(animeId: number, options: {
        limit?: number;
        offset?: number;
        sort?: "oldest" | "newest"
        ...
        return {
            ...
            next: (res.next as string | null) || null,
        };
    }
    ...
    async getRecentVideo(episodeId: number): Promise<Models.StreamInfo | null> {
        const res = await this._request<Raw.EpisodesV1IDRecentVideo>(`/episodes/v1/${episodeId}/recent-video/`);
        if ("code" in res && res.code === "INVALID") return null;
        return Mappers.mapStreamInfo(res as Raw.StreamingInfoV2);
    }

    /** PallyCon 서버에 접근하기 위한 Widevine 토큰을 얻어옵니다 (pallycon-customdata-v2 헤더용
     * @param ignoreLimit - True인 경우, 자동으로 updatePlayback 호출하여 플레이어를 종료한 것으로 서버를 속입니다.
    ) */
    async getVideoStream(episodeId: number, ignoreLimit: boolean = false): Promise<Models.StreamInfo> {
        const raw = await this._request<Raw.StreamingInfoV2>(
            `/episodes/v3/${episodeId}/video/?device=Web`,
            {},
            true
        );
        if (ignoreLimit && 'play_log_id' in raw && raw.play_log_id) await this.updatePlayback(raw.play_log_id, { exited: true, paused: true }); // 씨이발 타입좆병신 걍 raw.play_log_id면 충분하잖아
     ...
     * @param {boolean} [status.exited=true] - 종료 여부
     * @param {boolean} [status.paused=true] - 일시정지 여부
     * @returns {Promise<boolean>} 업데이트 성공 여부
     */
    async updatePlayback(playLogId: number, status: {
        playTime?: string;
        offset?: string;
        exited?: boolean;
        paused?: boolean;
    }): Promise<boolean> {
        return (await this._request<Response>(`/play_logs/${playLogId}/`, {
            method: "PATCH",
            body: JSON.stringify({
    ...
    async autoComplete(keyword: string): Promise<string[]> {
        return this._request<string[]>(`/search/v1/auto_complete/?keyword=${encodeURIComponent(keyword)}`);
    }

    async search(keyword: string): Promise<Models.Paginated<Models.Anime>> {
        const res = await this._request<Raw.SearchV3Keyword>(`/search/v3/keyword/?keyword=${encodeURIComponent(keyword)}`);
        return {
            total: res.count ?? 0,
    ...
    async getComments(episodeId: number, options: {
        limit?: number;
        offset?: number;
        sorting?: string
        ...
        return {
            total: res.count ?? 0,
            items: (res.results ?? []).map(Mappers.mapInteraction) as Models.Comment[],
            next: (res.next as string | null) || null,
        };
    }

    async addComment(episodeId: number, content: string, options: {
        isSpoiler?: boolean;
        parentId?: number
    ...
    async editComment(commentId: number, content: string, isSpoiler?: boolean): Promise<Models.Comment> {
        const res = await this._request<Raw.CommentItem>(`/comments/v1/${commentId}/`, {
            method: "PATCH",
            body: JSON.stringify({
                content,
    ...
    async deleteComment(commentId: number): Promise<boolean> {
        return (await this._request<Response>(`/comments/v1/${commentId}/`, { method: "DELETE" }, true, true))?.status === 204;
    }

    async likeComment(commentId: number, isActive: boolean = true): Promise<boolean> {
        return (await this._request<Response>(`/comments/v1/${commentId}/like/`, {
            method: "PATCH",
    ...
    async getReviews(animeId: number, options: {
        limit?: number;
        offset?: number;
        sorting?: string
    } = {}): Promise<Models.Paginated<Models.Review>> {
        const { limit = 10, offset = 0, sorting = "top" } = options;
        const res = await this._request<Raw.PaginatedResponse<Raw.ReviewsV1MyReview>>(
    ...
    async addReview(animeId: number, content: string, score: number, options: {
        isSpoiler?: boolean
    } = {}): Promise<Models.Review> {
        const res = await this._request<Raw.ReviewsV1MyReview>("/reviews/v1/list/", {
            method: "POST",
            body: JSON.stringify({
                item: animeId,
                content,
    ...
    async editReview(reviewId: number, score: number, content: string, isSpoiler?: boolean): Promise<Models.Review> {
        const res = await this._request<Raw.ReviewsV1MyReview>(`/reviews/v1/${reviewId}/`, {
            method: "PATCH",
            body: JSON.stringify({
                score,
                content,
    ...
    async likeReview(reviewId: number, isActive: boolean = true): Promise<boolean> {
        await this._request(`/reviews/v1/${reviewId}/like/`, {
            method: "PATCH",
            body: JSON.stringify({ is_active: isActive }),
        }, true);
        return true;
    ...
    /**@private */
    private async _request<T>(path: string, options: RequestInit = {}, forceToken = false, getRaw = false): Promise<T> {
        ...
        const isCommunity = path.includes("/reviews/") || path.includes("/comments/");

        if (this.config.token && (forceToken || isMutation || isVideo || isCommunity)) {
        ...
        if (contentType.includes("text/html")) {
            ...
            throw new Error(`WTF (possibly geo-restricted)\nStatus: ${response.status}\nBody: ${text}`);
        }
        ...
        if (!response.ok) {
            ...
            if (errorData.code === "BLOCKED_BY_DISALLOWED_ACCESS") {
                throw new Error("Geo-blocked: Content is not available outside of South Korea");
            }
            if (errorData.code === "PLAYBACK_EXPIRED") {
                throw new Error("Playback expired: The playback session has timed out");
            }

            throw new Error(`Request failed: ${response.status} ${response.statusText}\nEndpoint: ${path}\nResponse: ${JSON.stringify(errorData, null, 2)}`); // json 아니면 인생 망함
        }
